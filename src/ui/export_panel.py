...
class ExportPanel(ctk.CTkFrame):
    ...
    def _export(self):
        ...
        if clips:
            self._start_multi(clips, detection)
        else:
            self._start_single(detection)

    # ── single ─────────────────────────────────────────────────────────────

    def _start_single(self, detection: dict | None):
        ...
        target = self._run_detection if detection else self._run_plain
        threading.Thread(
            target=target, args=([(start, end)], [out], detection), daemon=True
        ).start()

    # ── multi ──────────────────────────────────────────────────────────────

    def _start_multi(self, clips: list[tuple[float, float]], detection: dict | None):
        ...
        target = self._run_detection if detection else self._run_plain
        threading.Thread(
            target=target, args=(clips, paths, detection), daemon=True
        ).start()

    # ── workers ────────────────────────────────────────────────────────────

    def _run_plain(self, clips, paths, _detection):
        total = len(clips)
        try:
    ...
    def _run_detection(self, clips, paths, detection):
        total = len(clips)
        try:
            for i, ((start, end), path) in enumerate(zip(clips, paths)):
